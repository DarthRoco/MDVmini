...
var natfreq=Math.sqrt(-k/block.mass);
var cc=2*block.mass*natfreq;
...
var b=cc*bratio;
    ...
    wall.t += frameRate;
    wall.lx = wall.x;
    wall.x = 30 + amp * Math.sin(2 * Math.PI * wall.frequency * wall.t);
    ...
    ctx.stroke();
    ctx.fillStyle = 'green';
    ctx.fillRect((block.x)/2-5, height / 2 + 5, ((block.x)/2-30),10);
    ctx.closePath();
...
var setup = function() {
    ...
    document.getElementById('b_slider').onchange = function() {
        this.innerHTML = this.value;
        bratio = -1 * parseFloat(this.value);
        document.getElementById('b_slider_label').innerHTML = -bratio;
    };
    ...
    document.getElementById('a_slider').onchange = function() {
        ...
        document.getElementById('a_slider_label').innerHTML = amp;
    };
